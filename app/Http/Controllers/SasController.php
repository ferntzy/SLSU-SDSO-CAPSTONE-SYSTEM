...
class SasController extends Controller
{
  ...
  {
   $pendingReviews = EventApprovalFlow::where('approver_role', 'SAS_Director')
    ...
    $rejected = EventApprovalFlow::where('approver_role', 'VP_sas')
      ->where('status', 'rejected')
      ->count();

    return view('sas.dashboard', compact('pendingReviews', 'approved', 'rejected'));
  }
  ...
  $pendingReviews = EventApprovalFlow::with(['permit.organization'])
    ...
    return view('sas.events.pending', compact('pendingReviews'));
  }
  ...
  public function approved()
  {
    ...
    return view('sas.events.approved', compact('approvedReviews'));
  }
  ...
  public function rejected()
  {
    ...
    return view('sas.events.rejected', compact('rejectedReviews'));
  }
  ...
  public function history()
  {
    ...
    return view('sas.events.history', compact('historyReviews'));
  }
  ...
  public function approve(Request $request, $approval_id)
{
    ...
    // Security check
    if ($flow->approver_role !== 'SAS_Director' || $flow->status !== 'pending') {
    ...
    // Save signed PDF
    $signedPath = $tempDir . "/sas_signed_{$permit->hashed_id}.pdf";
  ...
  public function reject(Request $request, $approval_id)
  {
    ...
    $flow->update([
      ...
      'approver_id' => Auth::id(),
      'approver_name' => strtoupper(Auth::user()->name ?? 'SAS_Director'),
      'updated_at' => now(),
    ]);
